...
const CouponList = () => {
  ...
  const fetchCoupons = async () => {
    try {
      ...
      console.log("Fetched coupons:", res.data); // Debug log
          ...
          <tbody className="text-gray-700 text-sm">
            {coupons.map((coupon) => (
              <tr key={coupon._id} className="hover:bg-gray-50 transition border-b">
                ...
                <td className="px-6 py-4">{coupon.discount ? `${coupon.discount}%` : "N/A"}</td>
                <td className="px-6 py-4">
                  {coupon.startDate ? coupon.startDate.substring(0, 10) : "N/A"}
                </td>
                <td className="px-6 py-4">
                  {coupon.endDate ? coupon.endDate.substring(0, 10) : "N/A"}
                </td>

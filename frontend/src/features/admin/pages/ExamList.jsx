  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setView("addExam")}
      ...
      <div className="overflow-x-auto">
        <table className="w-full table-auto border-collapse shadow-sm rounded-md overflow-hidden text-sm">
          ...
          <tbody className="text-gray-800">
            {exams.map((exam) => (
              <tr key={exam._id} className="hover:bg-gray-50 border-b transition">
                ...
                <td className="p-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${exam.status === "published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                    {exam.status === "published" ? "Published" : "Unpublished"}
                  </span>
                </td>
                <td className="p-3 space-x-2">
                  <button onClick={() => { setSelectedExam(exam); setView("editExam"); }}
                    className="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded text-xs">
                    Edit
                  </button>
                  <button onClick={() => { setSelectedExam(exam); setView("manageQuestions"); }}

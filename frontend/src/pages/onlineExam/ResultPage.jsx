...
const ResultPage = () => {
  ...
  useEffect(() => {
    ...
    const saveResult = async () => {
      try {
        const correct = state.totalQuestions - state.wrong;
  ...
  if (saving) return <p className="saving-msg">Saving your result...</p>;

  const isPass = state.percentage >= 33;
    ...
    <div className="result-page">
      ...
      <div className="result-grid">
        ...
        <div className="row">
          <div className="card">
            <h3>Completed At</h3>
            <p>{new Date(state.completedAt).toLocaleString()}</p>
          </div>
        </div>
      </div>

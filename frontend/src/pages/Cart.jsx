...
const Cart = () => {
  ...
  const handlePayment = async (gateway) => {
    try {
      ...
      if (gateway === 'razorpay') {
        const response = await axios.post('http://localhost:8000/api/payments/razorpay/create-order', orderData);
        const options = {
key: import.meta.env.VITE_RAZORPAY_KEY_ID || "rzp_test_7kAotmP1o8JR8V",
        ...
        const response = await axios.post('http://localhost:8000/api/payments/stripe/create-session', orderData);
